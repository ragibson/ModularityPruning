...
layout = G.layout_fruchterman_reingold(maxiter=10000, repulserad=3 * N ** 3)
...
for i, m in enumerate([membership0, membership1, membership2, membership3]):
    out = ig.plot(louvain.RBConfigurationVertexPartition(G, initial_membership=m), layout=layout, bbox=(600, 600))
    out.save("community_scales{}.png".format(i))
