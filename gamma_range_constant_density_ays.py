...
for N in [150, 300, 600]:
    B = N // 3
    for m_out2 in np.linspace(1, 5, 2):
        progress = Progress(ITERATIONS, name="N={}, m_out={:.1f}:".format(N, m_out2))
        theory_g2s = []
        theory_g3s = []
        observed_g2s = []
        observed_g3s = []
        ...
        # plt.xlabel("gamma")
        # plt.title("\"Are you sure?\" -- gamma range, N={}, m_out={:.1f}".format(N, m_out2))
        # plt.show()
